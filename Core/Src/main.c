...
static uint8_t ev_div=0;        //print every 10 EV ticks
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	  ...
	  if(htim->Instance==TIM3) sensor_flag=1;  //100ms EV tick
}
...
int main(void)
{
  ...
  HAL_TIM_Base_Start(&htim2);       /* count every 1 us     */

  //start interrupt on receiving data
  HAL_UART_Receive_IT(&huart1,&rx_byte,1);


  	  EV_Init(&ev);
  	  HCSR04_Init();
  	  ADAS_Init(&adas);   //initialize l f r as --> 400cm
  ...
  /* USER CODE BEGIN WHILE */
	  ...
	  while (1)
	  {
         ...
         if (sensor_flag==1)
         {
        	 ...
        	 //update TTC, blind spot detection, alarms
        	 ADAS_Update(&adas,&ev);
            //validating motor temperature, soc, collision based on this update the fault flag
        	 Fault_Check(&flt,&ev,&adas);
        	 //waiting to receiving the data process the cmnd
        	 Shell_Process();
...
{
	char msg[100];
	//split the floats into integer+decimal parts
	...
	/* Line 1: EV data */
     sprintf(msg,"SPD:%d.%d SOC:%d.%d TRQ:%d TMP:%d.%d RNG:%d ACC:%d BRK:%d\r\n",
		                      spd, spdd,
		                      soc, socd,
		                      trq,
		                      tmp, tmpd,
	...
	//line2: ADAS+fault
     sprintf(msg,"F:%d L:%d R:%d TTC:%d.%ds COL:%d BSD:%d%d ALM:%d FLT:%02X cm\r\n",frnt,left,right,ttcs,
		                          			 ttcd,adas.collision_warn,adas.blindspot_left,adas.blindspot_right,(int)adas.alarm_priority,flt.flags);
